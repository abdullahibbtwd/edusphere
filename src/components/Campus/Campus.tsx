...
const Campus = ({ school }: { school: string }) => {
  ...
  const displayCampusImages = campusImages.length > 0
    ? campusImages
      ...
      .filter(c => c.imageUrl) // Only show campus images with actual image URLs
    : [];
  ...
  const handleNext = () => {
    setActiveIndex((prev) => (prev + 1) % displayCampusImages.length);
  };

  const handlePrev = () => {
    setActiveIndex((prev) => (prev - 1 + displayCampusImages.length) % displayCampusImages.length);
  };
  ...
  const getCardStyle = (index: number) => {
    const position = (index - activeIndex + displayCampusImages.length) % displayCampusImages.length;
    ...
    if (position === 0) {
    ...
    } else if (position === displayCampusImages.length - 1) {
      ...
      // Hidden cards
      return {
        x: position < displayCampusImages.length / 2 ? 400 : -400,
        scale: 0.6,
        opacity: 0,
        zIndex: 10,
        rotateY: 0,
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="md:text-xl text-sm md:text-xl text-muted max-w-2xl mx-auto">
            Explore the beautiful spaces that make our campus home
          </p>
        </motion.div>
        ...
        {displayCampusImages.length > 1 && (
          <div className="flex justify-center gap-3">
            {displayCampusImages.map((_, index) => (
              <button
                key={index}
        ...
        >
          <p className="text-sm font-bold text-muted uppercase tracking-widest">
            {activeIndex + 1} / {displayCampusImages.length}
          </p>
        </motion.div>
      </div>
    </section>
